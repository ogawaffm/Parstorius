...
public abstract class IntegralNumberFormatterParseDefaultsTester<T extends Number>
    implements FormatterParseDefaultsTester<T, NumberFormatter<T>> {

  Class<T> classT;
  Function<Integer, T> caster;
  IntegralNumberFormatterParseDefaultsTester(Class<T> classT, Function<Integer, T> caster) {
    this.classT = classT;
    this.caster = caster;
  }
  ...
  @Override public T getTestParseDefault(final PARSE_RESULT_CAUSE formatDefault) {
    return caster.apply(-formatDefault.ordinal() -1);
  }
}
